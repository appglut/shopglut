...
class QuickViewchooseTemplates {

	public function __construct() {
		add_action('admin_post_create_quickview_layout', array($this, 'handleCreateQuickviewEnhancement'));
		add_action('wp_ajax_get_quickview_demo_content', array($this, 'handleGetQuickviewDemoContent'));
	}

	public function loadProductQuickviewTemplates() {
		$tab_names = [
			'tab1' => [
						...
						<div class="shopg-template-preview" data-template="<?php echo esc_attr($layout_template); ?>">
							...
							<div class="template-content product-demo">
								...
								<div class="template-html-container">
									...
									<div class="html-preview-background">
										<?php $this->renderQuickviewPreview($layout_template); ?>
									</div>

									<!-- Foreground: View Demo Button with Overlay -->
									<div class="html-preview-overlay">
										<button type="button" class="demo-view-btn" onclick="openHtmlDemoModal('<?php echo esc_attr($layout_template); ?>', 'product-quickview')">
							...
							<div class="template-footer">
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									<input type="hidden" name="action" value="create_quickview_layout">
									<input type="hidden" name="layout_template" value="<?php echo esc_attr($layout_template); ?>">
									<?php wp_nonce_field('create_quickview_layout_nonce', 'create_quickview_layout_nonce'); ?>
		...
		.shopg-template-gallery .html-preview-background .quickview-modal-content,
		.shopg-template-gallery .html-preview-background .shopglut-product-quickview .quickview-modal-content {
		...
		/* Override quickview modal styles in preview */
		.shopg-template-gallery .html-preview-background .quickview-modal {
			position: static !important;
		...
		/* Ensure the quickview preview container is centered and fits properly */
		.shopg-template-gallery .html-preview-background .shopglut-quickview-preview-container {
			position: absolute !important;
			display: flex !important;
			align-items: center !important;
			justify-content: center !important;
		...
		/* Hide all scrollbars in the demo preview */
		...
		.shopg-template-gallery .html-preview-background .quickview-modal,
		.shopg-template-gallery .html-preview-background .quickview-modal-content,
		.shopg-template-gallery .html-preview-background .quickview-inner {
			overflow: hidden !important;
			max-height: none !important;
		...
		.shopg-template-gallery .html-preview-background .shopglut-product-quickview {
			overflow: hidden !important;
		...
		.shopg-template-gallery .html-preview-background .quickview-gallery,
		.shopg-template-gallery .html-preview-background .quickview-info {
			overflow: hidden !important;
			max-height: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .quickview-modal-overlay {
			display: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .quickview-close {
			display: none !important;
		}

		/* Fix Live Demo modal visibility issue - override template1 style conflicts */
		#htmlDemoModal .shopglut-product-quickview.template1 .quickview-modal {
			opacity: 1 !important;
			visibility: visible !important;
			position: fixed !important;
			display: flex !important;
		...
		function testQuickviewAjax() {
			fetch(ajax_url, {
				...
				body: new URLSearchParams({
					action: 'get_quickview_demo_content',
		...
		// Test on page load
		setTimeout(testQuickviewAjax, 2000);
				...
				if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
					mutation.addedNodes.forEach(function(node) {
						if (node.nodeType === 1) { // Element node
							...
							if (node.classList && node.classList.contains('shopglut-product-quickview') ||
								(node.id && node.id === 'htmlDemoModal') ||
								(node.querySelector && node.querySelector('.shopglut-product-quickview'))) {
		...
		// Keep image modal functionality for quickview images
		function openImageModal(imageName) {
			const modal = document.getElementById('imageModal');
			const mainImage = document.getElementById('modalMainImage');
			const assetsUrl = '<?php echo esc_js(SHOPGLUT_URL . "global-assets/images/quickview-templates/"); ?>';
	 ...
	 */
	private function renderQuickviewPreview($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/enhancements/ProductQuickView/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/enhancements/ProductQuickView/templates/template1/template1Style.php';

		$markup = new \Shopglut\enhancements\ProductQuickView\templates\template1\template1Markup();
		$style = new \Shopglut\enhancements\ProductQuickView\templates\template1\template1Style();

		// Render styles inline for preview
		$style->dynamicCss(0);
		?>
		<div class="shopglut-quickview-preview-container">
			<?php
			// Render the demo content directly from template1Markup
			// Get default settings for demo
			$settings = $this->getDefaultQuickviewSettings();
			$markup->render_demo_quickview($settings);
			?>
		</div>
		<?php
	}
	 ...
	 */
	private function getDefaultQuickviewSettings() {
		return array(
			'modal_width' => '1100px',
	 ...
	 */
	private function renderQuickviewFullDemo($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/enhancements/ProductQuickView/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/enhancements/ProductQuickView/templates/template1/template1Style.php';
		...
		// Render the full demo quickview
		$markup->render_demo_quickview($settings);
	}

	/**
	 * Handle AJAX request for quickview demo content
	 */
	public function handleGetQuickviewDemoContent() {
		// Verify nonce
		...
		$this->renderQuickviewFullDemo($template_id);
		wp_die();
	}

	public function handleCreateQuickviewEnhancement() {
		if (
			!isset($_POST['create_quickview_layout_nonce']) ||
			!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['create_quickview_layout_nonce'])), 'create_quickview_layout_nonce') ||
	...
	// Use $wpdb->insert() with proper error handling and caching
			...
			$cache_key = 'shopglut_quickview_layouts';
			...
			$update_result = $wpdb->query(
				$wpdb->prepare(
					"UPDATE `" . esc_sql($table_name) . "` SET layout_name = %s WHERE id = %d",
