...
(function($) {
    ...
    window.ShopGlutProductComparisonLayout = window.ShopGlutProductComparisonLayout || {};

    // Data converter module namespace
    ShopGlutProductComparisonLayout.dataConverter = {
        ...
        shouldSkipField: function(name) {
            ...
            // Skip non-productcomparison layout related fields
            var skipFields = [
                'agl_metabox_nonce',
                '_wp_http_referer',
                '_pseudo',
                'shopg_productcomparison_layouts_nonce',
                'shopg_shop_layoutid',
                'layout_name',
                'layout_template'
            ];
         ...
         * e.g., "shopg_productcomparison_settings_template1[tab][field]" -> {shopg_productcomparison_settings_template1: {tab: {field: value}}}
         */
        ...
        /**
         * Parse a field name like "shopg_productcomparison_settings_template1[tab][field][0]" into an array of keys
         */
        ...
        saveProductComparisonLayoutData: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'save_shopg_productcomparison_layoutdata',
                layout_name: layoutName,
                layout_template: layoutTemplate,
                shopg_productcomparison_layoutid: layoutId,
                productcomparison_nonce: nonce,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ShopGlutProductComparisonLayout.dataConverter.showNotification('success', response.data.message || 'Order complete layout saved successfully!');
                        ...
                        // Show error message
                        ShopGlutProductComparisonLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to save order complete layout.');
                    }
                },
                error: function(xhr) {
                    ...
                    ShopGlutProductComparisonLayout.dataConverter.showNotification('error', errorMessage);
                },
                complete: function() {
                    // Hide loader
         ...
         */
        resetProductComparisonLayoutSettings: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'reset_shopg_productcomparison_layout_settings',
                layout_id: layoutId,
                nonce: nonce
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Show success message
                        ShopGlutProductComparisonLayout.dataConverter.showNotification('success', response.data.message || 'Settings reset to default successfully!');
                        ...
                        // Show error message
                        ShopGlutProductComparisonLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to reset settings.');
                    }
                },
                error: function() {
                    ShopGlutProductComparisonLayout.dataConverter.showNotification('error', 'Network error occurred while resetting settings.');
                },
                complete: function() {
                    // Hide loader
                    $(".loader-overlay").css({"display": "none", "opacity": "0"});
    ...
    $(document).ready(function() {
        ...
        $('#productcomparisonLayout-publishing-action #publish').on('click', function(e) {
            e.preventDefault();
            ShopGlutProductComparisonLayout.dataConverter.saveProductComparisonLayoutData();
        });

        // Bind reset button click
        $('#productcomparison-reset-settings-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutProductComparisonLayout.dataConverter.resetProductComparisonLayoutSettings();
        });

        // Comparison display selection AJAX - following select_products pattern
        $(".agl-field.agl-field-select_comparison_display .agl-fieldset").on("click", function () {
            if ($(".agl-field.agl-field-select_comparison_display .chosen-drop .chosen-results").length > 0) {

                // Validate required data exists
                if (!shopglut_admin_vars || !shopglut_admin_vars.ajax_url) {
                    console.warn('Missing required data for comparison display options request');
                    return;
                }
                ...
                if (!nonce) {
                    console.warn('Missing nonce for comparison display options request');
                    return;
                }

                // Show loading spinner
                $(".agl-field.agl-field-select_comparison_display .chosen-drop").append('<div class="loading-spinner"></div>');

                $.ajax({
                    url: shopglut_admin_vars.ajax_url || ajaxurl,
                    method: "POST",
                    data: {
                        action: "shopglut_get_comparison_display_options",
                        nonce: nonce,
                        layout_id: layoutId,
                    },
                    ...
                    success: function(response) {
                        if (response.success && response.data) {
                            ...
                            $(".agl-field.agl-field-select_comparison_display .chosen-drop .chosen-results li").each(function() {
                                var itemText = $(this).text().trim();
                                // Remove the " (Used by another layout)" suffix for comparison
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.error("Failed to retrieve comparison display options:", {
                            status: textStatus,
                            error: errorThrown,
                        ...
                        if (jqXHR.status === 403) {
                            ShopGlutProductComparisonLayout.dataConverter.showNotification('error', 'Access denied. Please refresh the page.');
                        }
                    },
                    complete: function () {
                        // Remove loading spinner
                        $(".agl-field.agl-field-select_comparison_display .chosen-drop .loading-spinner").remove();
                        $(".agl-field.agl-field-select_comparison_display .chosen-results").removeClass("filter-dis");
                    }
                });
            }
        });
