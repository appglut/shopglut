...
class PackagingTemplates {
    
    public function render() {
        $current_template = get_option('agshopglut_pdf_invoices_packaging_template', 'default');
        ?>
        <div class="shopglut-packaging-templates-tab">
            <div class="templates-header">
                <h2><?php echo esc_html__('Packaging Slip Templates', 'shopglut'); ?></h2>
                <p><?php echo esc_html__('Select, edit and customize your packaging slip templates.', 'shopglut'); ?></p>
            </div>

            <div class="template-selection-section">
                <h3><?php echo esc_html__('Select Packaging Template', 'shopglut'); ?></h3>
        ...
        <style>
        .shopglut-packaging-templates-tab {
            background: #fff;
            padding: 20px;
            border-radius: 8px;
        ...
        jQuery(document).ready(function($) {
            ...
            function previewTemplate(templateId) {
                $.post(ajaxurl, {
                    action: 'shopglut_preview_packaging_template',
                    template: templateId,
            ...
            function editTemplate(templateId) {
                ...
                $.post(ajaxurl, {
                    action: 'shopglut_get_packaging_template',
                    template: templateId,
                    nonce: '<?php echo esc_js( wp_create_nonce('shopglut_template_nonce') ); ?>'
                }, function(response) {
                    if (response.success) {
            ...
            function saveTemplate() {
                ...
                $.post(ajaxurl, {
                    action: 'shopglut_save_packaging_template',
            ...
            function resetTemplate(templateId) {
                $.post(ajaxurl, {
                    action: 'shopglut_reset_packaging_template',
                    template: templateId,
                    nonce: '<?php echo esc_js( wp_create_nonce('shopglut_template_nonce') ); ?>'
                }, function(response) {
                    if (response.success) {
    ...
    private function getAvailableTemplates() {
        return array(
            'default' => array(
                ...
                'description' => __('Clean and professional default packaging slip template', 'shopglut'),
                'preview' => plugins_url('assets/images/templates/packaging-default.png', dirname(__FILE__, 2))
            ),
            'modern' => array(
                'name' => __('Modern Template', 'shopglut'),
                'description' => __('Modern design with bold typography and clean layout', 'shopglut'),
                'preview' => plugins_url('assets/images/templates/packaging-modern.png', dirname(__FILE__, 2))
            ),
            'classic' => array(
                'name' => __('Classic Template', 'shopglut'),
                'description' => __('Traditional business packaging slip with professional styling', 'shopglut'),
                'preview' => plugins_url('assets/images/templates/packaging-classic.png', dirname(__FILE__, 2))
            ),
            'minimal' => array(
                'name' => __('Minimal Template', 'shopglut'),
                'description' => __('Clean and minimal design focusing on essential information', 'shopglut'),
                'preview' => plugins_url('assets/images/templates/packaging-minimal.png', dirname(__FILE__, 2))
            )
        );
    }
}