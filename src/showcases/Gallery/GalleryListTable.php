...
class GalleryListTable extends \WP_List_Table {

	public function get_showcases( $per_page, $current_page = 1 ) {
		return GalleryEntity::retrieveAll( $per_page, $current_page );
	}

	public function get_totals() {
		return GalleryEntity::retrieveAllCount();
	}
	...
	public function no_items() {
		...
		echo '<h3 style="color: #374151; margin: 0 0 8px 0;">' . esc_html__( 'No Gallery Layouts Found', 'shopglut' ) . '</h3>';
		echo '<p style="color: #6b7280; margin: 0 0 20px 0;">' . esc_html__( 'Create your first Gallery layout to get started.', 'shopglut' ) . '</p>';
		echo '<a href="' . esc_url( admin_url( 'admin.php?page=shopglut_showcases&view=gallery_templates' ) ) . '" class="button button-primary">' . esc_html__( 'Create New Layout', 'shopglut' ) . '</a>';
		echo '</div>';
	}
	...
	public function column_layout_name( $item ) {
		...
		$actions = array(
			...
			'delete' => sprintf(
				...
				esc_url( $delete_link ),
				esc_html__( 'Are you sure you want to delete this Gallery layout?', 'shopglut' ),
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if ( ! empty( $layout_ids ) ) {
				foreach ( $layout_ids as $layout_id ) {
					GalleryEntity::delete_enhancement( $layout_id );
				}
				...
				add_action( 'admin_notices', function() use ( $layout_ids ) {
					...
					echo '<div class="notice notice-success is-dismissible"><p>' .
						sprintf(
							...
							esc_html( _n( '%d Shop Gallery layout deleted successfully.', '%d Shop Gallery layouts deleted successfully.', $count, 'shopglut' ) ),
							(int) $count
						) . '</p></div>';
				});
			}
	...
	public static function handle_individual_actions() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verified below
		if ( isset( $_GET['page'] ) && $_GET['page'] === 'shopglut_showcases' && isset( $_GET['view'] ) && $_GET['view'] === 'gallery' ) {
			...
			if ( isset( $_GET['action'] ) && $_GET['action'] === 'duplicate' && isset( $_GET['layout_id'] ) ) {
				...
				$original_layout = GalleryEntity::retrieve_by_id( $layout_id );
				if ( $original_layout ) {
					// Create duplicate with modified name
					/* translators: %s: original layout name */
					$duplicate_name = sprintf( esc_html__( '%s (Copy)', 'shopglut' ), $original_layout['layout_name'] );

					$new_layout_id = GalleryEntity::duplicate_enhancement( $layout_id, $duplicate_name );
	...
	private function add_table_styles() {
		?>
		<style>
			/* Gallery List Table Custom Styles */
