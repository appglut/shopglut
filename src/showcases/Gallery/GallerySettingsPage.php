...
class GallerySettingsPage {

	public $menu_slug = 'product_gallery';
...
$layout_data = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct query required for custom table operation
			sprintf("SELECT layout_name, layout_template FROM `%s` WHERE id = %d", esc_sql($table_name), $layout_id ) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Using sprintf with escaped table name, no prepare needed
		);
	    ...
	    <form id="shopglut_shop_layouts" method="post" action="">
				...
				<?php 
				$shopg_cpage_nonce = wp_create_nonce( 'shopg_Gallery_layouts' ); 
				?>
				<input type="hidden" name="shopg_Gallery_layouts_nonce" value="<?php echo esc_attr( $shopg_cpage_nonce ); ?>">
				...
				<div class="shopglut_layout_contents">
					...
					<div class="shopglut_editor_header">
						...
						<div class="back-to-menu">
							...
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=shopglut_showcases&view=gallerys' ) ); ?>"
			   ...
			   <div id="post-body" class="metabox-holder columns-2">
						...
						<div id="shopg-Gallery-layout-settings" class="postbox-container shopg-layout-settings-wrapper">
						...
						<div class="submitbox" id="submitpost">
								<div id="GalleryLayout-publishing-action" class="shopg-publishing-button">
									<button type="button" id="Gallery-reset-settings-button" class="btn btn-fullwidth btn-secondary"
									style=" background: #dc3545; color: white; border: none;">
									<?php echo esc_attr__( 'Reset', 'shopglut' ); ?>
									</button>
									<button type="button" name="publish" id="Gallery-save-layout-button" class="btn btn-fullwidth">
						...
						<div id="shopg-Gallery-layout-container" class="shopg-admin-edit-panel shopg-layout-container">
		...
		<style>
			...
			/* When loader is visible, prevent interaction with gallery demo */
			.loader-overlay:not([style*="display: none"]) ~ * .shopglut-product-gallery {
