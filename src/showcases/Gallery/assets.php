<?php
/**
 * Asset registration for product_gallery
 */
...
class product_galleryAssets {
    ...
    public function enqueue_frontend_assets() {
        ...
        // Enqueue JS
        if (file_exists(__DIR__ . '/assets/gallery-frontend.js')) {
            wp_enqueue_script(
                'product_gallery-script',
                $plugin_url . 'assets/gallery-frontend.js',
                ['jquery'],
                filemtime(__DIR__ . '/assets/gallery-frontend.js'),
                true
            );

            // Localize script with AJAX URL and other data
            wp_localize_script('product_gallery-script', 'shopglutGalleryData', array(
    ...
    public function enqueue_admin_assets($hook) {
        ...
        $is_product_gallery_editor = isset($_GET['editor']) && $_GET['editor'] === 'gallery';

        // Enqueue admin CSS
        if (file_exists(__DIR__ . '/assets/admin-style.css')) {
            wp_enqueue_style(
                'product_gallery-admin-style',
        ...
        if ($is_product_gallery_editor && file_exists(__DIR__ . '/assets/js/product_gallery-data-converter.js')) {
            wp_enqueue_script(
                'shopglut-product_gallery-enhancement-data-converter',
                $plugin_url . 'assets/js/product_gallery-data-converter.js',
                ['jquery'],
                filemtime(__DIR__ . '/assets/js/product_gallery-data-converter.js'),
                true
            );

            // Localize script with ajax URL and other data
            wp_localize_script('shopglut-product_gallery-data-converter', 'shopglut_admin_vars', array(
