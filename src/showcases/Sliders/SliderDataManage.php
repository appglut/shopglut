...
class SliderDataManage {

	public function __construct() {
		// AJAX handlers for product slider save and reset
		add_action('wp_ajax_save_shopg_Slider_layoutdata', [$this, 'save_Slider_layout_data']);
		add_action('wp_ajax_reset_shopg_Slider_layout_settings', [$this, 'reset_Slider_layout_settings']);

		// Handle individual actions (delete, duplicate)
		add_action('admin_init', [SliderListTable::class, 'handle_individual_actions']);

		// Handle success messages
		add_action('admin_notices', [$this, 'show_action_messages']);

		// Initialize slider button display
	}


	/**
	 * Save product slider layout data from AJAX
	 */
	public function save_Slider_layout_data() {
		...
		if (!isset($_POST['Slider_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['Slider_nonce'])), 'shopg_Slider_layouts')) {
		...
		$layout_id = isset($_POST['shopg_Slider_layoutid']) ? intval(sanitize_text_field(wp_unslash($_POST['shopg_Slider_layoutid']))) : 0;
		...
		$preview_html = $this->shopglut_render_slider_preview($layout_id);

		wp_send_json_success(array(
			'message' => 'Product slider layout saved successfully',
			'layout_id' => $layout_id,
			'html' => $preview_html
		));
	}
	...
	/**
	 * Reset product slider layout settings to default
	 */
	public function reset_Slider_layout_settings() {
		// Check nonce
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'shopg_Slider_layouts')) {
			wp_send_json_error('Invalid nonce');
			return;
		}
		...
		$cache_key = "shopglut_slider_layout_{$layout_id}";
		$layout_data = wp_cache_get( $cache_key, 'shopglut_slider' );

		if ( false === $layout_data ) {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Direct query required for custom table operation
			$layout_data = $wpdb->get_row(
				$wpdb->prepare( "SELECT layout_template FROM %i WHERE id = %d", $table_name, $layout_id ) // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnsupportedIdentifierPlaceholder -- WP 6.2+ identifier placeholder supported
			);

			// Cache the result for 30 minutes
			wp_cache_set( $cache_key, $layout_data, 'shopglut_slider', 30 * MINUTE_IN_SECONDS );
		}
		...
		/**
     * Render product slider layout preview
     */
    public function shopglut_render_slider_preview( $layout_id = 0 ) {
        ...
        if ( false === $layout_data ) {
            ...
            $layout_data = $wpdb->get_row(
                $wpdb->prepare( "SELECT * FROM `{$wpdb->prefix}shopglut_slider_layouts` WHERE id = %d", $layout_id )
            );
        ...
        $markup_class = 'Shopglut\\showcases\\Slider\\templates\\' . $template_name . '\\' . $template_name . 'Markup';
        if ( ! class_exists( $markup_class ) ) {
            return '<div class="shopglut-preview-error">Markup class not found: ' . esc_html( $markup_class ) . '</div>';
        }

        // Get the style class
        $style_class = 'Shopglut\\showcases\\Slider\\templates\\' . $template_name . '\\' . $template_name . 'Style';
        ...
        try {
            ...
            echo '<div class="shopglut-slider-preview-wrapper">';
            echo '<style>.shopglut-product-slider.template1 .slider-modal { opacity: 1; visibility: visible; position: relative; }</style>';
	...
	public function show_action_messages() {
		...
		if ( isset( $_GET['deleted'] ) && $_GET['deleted'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Slider layout deleted successfully.', 'shopglut' ) .
				'</p></div>';
		}
		...
		if ( isset( $_GET['duplicated'] ) && $_GET['duplicated'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Slider layout duplicated successfully.', 'shopglut' ) .
				'</p></div>';
		}
	}
