...
class SliderchooseTemplates {

	public function __construct() {
		add_action('admin_post_create_slider_layout', array($this, 'handleCreateSliderEnhancement'));
		add_action('wp_ajax_get_slider_demo_content', array($this, 'handleGetSliderDemoContent'));
	}

	public function loadSliderTemplates() {
		$tab_names = [
			'tab1' => [
						...
						<div class="shopg-template-preview" data-template="<?php echo esc_attr($layout_template); ?>">
							...
							<div class="template-content product-demo">
								...
								<div class="template-html-container">
									...
									<div class="html-preview-background">
										<?php $this->renderSliderPreview($layout_template); ?>
									</div>

									<!-- Foreground: View Demo Button with Overlay -->
									<div class="html-preview-overlay">
										<button type="button" class="demo-view-btn" onclick="openHtmlDemoModal('<?php echo esc_attr($layout_template); ?>', 'product-slider')">
							...
							<div class="template-footer">
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									<input type="hidden" name="action" value="create_slider_layout">
									<input type="hidden" name="layout_template" value="<?php echo esc_attr($layout_template); ?>">
									<?php wp_nonce_field('create_slider_layout_nonce', 'create_slider_layout_nonce'); ?>
		...
		.shopg-template-gallery .html-preview-background .slider-modal-content,
		.shopg-template-gallery .html-preview-background .shopglut-product-slider .slider-modal-content {
		...
		/* Override slider modal styles in preview */
		.shopg-template-gallery .html-preview-background .slider-modal {
			position: static !important;
		...
		/* Ensure the slider preview container is centered and fits properly */
		.shopg-template-gallery .html-preview-background .shopglut-slider-preview-container {
			position: absolute !important;
			display: flex !important;
			align-items: center !important;
			justify-content: center !important;
		...
		.shopg-template-gallery .html-preview-background .slider-modal,
		.shopg-template-gallery .html-preview-background .slider-modal-content,
		.shopg-template-gallery .html-preview-background .slider-inner {
			overflow: hidden !important;
			max-height: none !important;
		...
		.shopg-template-gallery .html-preview-background .shopglut-product-slider {
			overflow: hidden !important;
		...
		.shopg-template-gallery .html-preview-background .slider-gallery,
		.shopg-template-gallery .html-preview-background .slider-info {
			overflow: hidden !important;
			max-height: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .slider-modal-overlay {
			display: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .slider-close {
			display: none !important;
		}

		/* Fix Live Demo modal visibility issue - override template1 style conflicts */
		#htmlDemoModal .shopglut-product-slider.template1 .slider-modal {
			opacity: 1 !important;
			visibility: visible !important;
			position: fixed !important;
			display: flex !important;
		...
		function testSliderAjax() {
			fetch(ajax_url, {
				...
				body: new URLSearchParams({
					action: 'get_slider_demo_content',
		...
		// Test on page load
		setTimeout(testSliderAjax, 2000);
		...
		// Keep image modal functionality for slider images
		function openImageModal(imageName) {
			const modal = document.getElementById('imageModal');
			const mainImage = document.getElementById('modalMainImage');
			const assetsUrl = '<?php echo esc_js(SHOPGLUT_URL . "global-assets/images/slider-templates/"); ?>';
	 ...
	 */
	private function renderSliderPreview($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Sliders/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Sliders/templates/template1/template1Style.php';

		$markup = new \Shopglut\showcases\Sliders\templates\template1\template1Markup();
		$style = new \Shopglut\showcases\Sliders\templates\template1\template1Style();

		// Render styles inline for preview
		$style->dynamicCss(0);
		?>
		<div class="shopglut-slider-preview-container">
			<?php
			// Render the demo content directly from template1Markup
			// Get default settings for demo
			$settings = $this->getDefaultSliderSettings();
			$markup->render_slider($settings, 0);
			?>
		</div>
		<?php
	}
	...
	/**
	 * Get default Slider settings for demo
	 */
	private function getDefaultSliderSettings() {
		return array(
	 ...
	 */
	private function renderSliderFullDemo($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Sliders/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Sliders/templates/template1/template1Style.php';
		...
		// Render the full demo slider
		$markup->render_slider($settings, 0);
	}

	/**
	 * Handle AJAX request for slider demo content
	 */
	public function handleGetSliderDemoContent() {
		// Verify nonce
		...
		$this->renderSliderFullDemo($template_id);
		wp_die();
	}

	public function handleCreateSliderEnhancement() {
		if (
			!isset($_POST['create_slider_layout_nonce']) ||
			!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['create_slider_layout_nonce'])), 'create_slider_layout_nonce') ||
	...
	// Use $wpdb->insert() with proper error handling and caching
			...
			$cache_key = 'shopglut_slider_layouts';
			...
			$update_result = $wpdb->query(
				$wpdb->prepare(
					"UPDATE %i SET layout_name = %s WHERE id = %d", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnsupportedIdentifierPlaceholder -- WP 6.2+ identifier placeholder supported
					$table_name,
