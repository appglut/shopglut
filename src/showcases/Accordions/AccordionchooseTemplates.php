...
class AccordionchooseTemplates {

	public function __construct() {
		add_action('admin_post_create_accordion_layout', array($this, 'handleCreateAccordionEnhancement'));
		add_action('wp_ajax_get_accordion_demo_content', array($this, 'handleGetAccordionDemoContent'));
	}

	public function loadAccordionTemplates() {
		$tab_names = [
			'tab1' => [
						...
						<div class="shopg-template-preview" data-template="<?php echo esc_attr($layout_template); ?>">
							...
							<div class="template-content product-demo">
								...
								<div class="template-html-container">
									...
									<div class="html-preview-background">
										<?php $this->renderAccordionPreview($layout_template); ?>
									</div>

									<!-- Foreground: View Demo Button with Overlay -->
									<div class="html-preview-overlay">
										<button type="button" class="demo-view-btn" onclick="openHtmlDemoModal('<?php echo esc_attr($layout_template); ?>', 'product-accordion')">
							...
							<div class="template-footer">
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									<input type="hidden" name="action" value="create_accordion_layout">
									<input type="hidden" name="layout_template" value="<?php echo esc_attr($layout_template); ?>">
									<?php wp_nonce_field('create_accordion_layout_nonce', 'create_accordion_layout_nonce'); ?>
		...
		.shopg-template-gallery .html-preview-background .accordion-modal-content,
		.shopg-template-gallery .html-preview-background .shopglut-product-accordion .accordion-modal-content {
		...
		/* Override accordion modal styles in preview */
		.shopg-template-gallery .html-preview-background .accordion-modal {
			position: static !important;
		...
		/* Ensure the accordion preview container is centered and fits properly */
		.shopg-template-gallery .html-preview-background .shopglut-accordion-preview-container {
			position: absolute !important;
			display: flex !important;
			align-items: center !important;
			justify-content: center !important;
		...
		.shopg-template-gallery .html-preview-background .accordion-modal,
		.shopg-template-gallery .html-preview-background .accordion-modal-content,
		.shopg-template-gallery .html-preview-background .accordion-inner {
			overflow: hidden !important;
			max-height: none !important;
		...
		.shopg-template-gallery .html-preview-background .shopglut-product-accordion {
			overflow: hidden !important;
		...
		.shopg-template-gallery .html-preview-background .accordion-gallery,
		.shopg-template-gallery .html-preview-background .accordion-info {
			overflow: hidden !important;
			max-height: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .accordion-modal-overlay {
			display: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .accordion-close {
			display: none !important;
		}

		/* Fix Live Demo modal visibility issue - override template1 style conflicts */
		#htmlDemoModal .shopglut-product-accordion.template1 .accordion-modal {
			opacity: 1 !important;
			visibility: visible !important;
			position: fixed !important;
			display: flex !important;
		...
		function testAccordionAjax() {
			fetch(ajax_url, {
				...
				body: new URLSearchParams({
					action: 'get_accordion_demo_content',
		...
		// Test on page load
		setTimeout(testAccordionAjax, 2000);
				...
				if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
					mutation.addedNodes.forEach(function(node) {
						if (node.nodeType === 1) { // Element node
							...
							if (node.classList && node.classList.contains('shopglut-product-accordion') ||
								(node.id && node.id === 'htmlDemoModal') ||
								(node.querySelector && node.querySelector('.shopglut-product-accordion'))) {
		...
		// Keep image modal functionality for accordion images
		function openImageModal(imageName) {
			const modal = document.getElementById('imageModal');
			const mainImage = document.getElementById('modalMainImage');
			const assetsUrl = '<?php echo esc_js(SHOPGLUT_URL . "global-assets/images/accordion-templates/"); ?>';
	 ...
	 */
	private function renderAccordionPreview($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Accordions/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Accordions/templates/template1/template1Style.php';

		$markup = new \Shopglut\showcases\Accordions\templates\template1\template1Markup();
		$style = new \Shopglut\showcases\Accordions\templates\template1\template1Style();

		// Render styles inline for preview
		$style->dynamicCss(0);
		?>
		<div class="shopglut-accordion-preview-container">
			<?php
			// Render the demo content directly from template1Markup
			// Get default settings for demo
			$settings = $this->getDefaultAccordionSettings();
			$markup->render_demo_accordion($settings, []);
			?>
		</div>
		<?php
	}
	 ...
	 */
	private function getDefaultAccordionSettings() {
		return array(
			'modal_width' => '1100px',
	 ...
	 */
	private function renderAccordionFullDemo($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Accordions/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Accordions/templates/template1/template1Style.php';
		...
		// Render the full demo accordion
		$markup->render_demo_accordion($settings, []);
	}

	/**
	 * Handle AJAX request for accordion demo content
	 */
	public function handleGetAccordionDemoContent() {
		// Verify nonce
		...
		$this->renderAccordionFullDemo($template_id);
		wp_die();
	}

	public function handleCreateAccordionEnhancement() {
		if (
			!isset($_POST['create_accordion_layout_nonce']) ||
			!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['create_accordion_layout_nonce'])), 'create_accordion_layout_nonce') ||
	...
	// Use $wpdb->insert() with proper error handling and caching
			...
			$cache_key = 'shopglut_accordion_layouts';
			wp_cache_delete($cache_key, 'shopglut');

			// Update the layout_name with the actual ID using $wpdb->prepare()
			...
			$update_result = $wpdb->query(
				$wpdb->prepare(
					sprintf("UPDATE `%s` SET layout_name = %s WHERE id = %d", $escaped_table), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- Using sprintf with escaped table name
					sanitize_text_field('Layout(#' . $layout_id . ')'),
					$layout_id
				)
			);
