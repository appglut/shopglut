...
class AccordionDataManage {

	public function __construct() {
		// AJAX handlers for product accordion save and reset
		add_action('wp_ajax_save_shopg_Accordion_layoutdata', [$this, 'save_Accordion_layout_data']);
		add_action('wp_ajax_reset_shopg_Accordion_layout_settings', [$this, 'reset_Accordion_layout_settings']);

		// Handle individual actions (delete, duplicate)
		add_action('admin_init', [AccordionListTable::class, 'handle_individual_actions']);

		// Handle success messages
		add_action('admin_notices', [$this, 'show_action_messages']);

		// Initialize accordion button display
	}


	/**
	 * Save product accordion layout data from AJAX
	 */
	public function save_Accordion_layout_data() {
		...
		if (!isset($_POST['Accordion_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['Accordion_nonce'])), 'shopg_Accordion_layouts')) {
		...
		$layout_id = isset($_POST['shopg_Accordion_layoutid']) ? intval(sanitize_text_field(wp_unslash($_POST['shopg_Accordion_layoutid']))) : 0;
		...
		$preview_html = $this->shopglut_render_accordion_preview($layout_id);

		wp_send_json_success(array(
			'message' => 'Product accordion layout saved successfully',
			'layout_id' => $layout_id,
			'html' => $preview_html
		));
	}
	...
	/**
	 * Reset product accordion layout settings to default
	 */
	public function reset_Accordion_layout_settings() {
		// Check nonce
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'shopg_Accordion_layouts')) {
			wp_send_json_error('Invalid nonce');
			return;
		}
		...
		$cache_key = "shopglut_accordion_layout_{$layout_id}";
		$layout_data = wp_cache_get( $cache_key, 'shopglut_accordion' );
		...
		if ( false === $layout_data ) {
			...
			wp_cache_set( $cache_key, $layout_data, 'shopglut_accordion', 30 * MINUTE_IN_SECONDS );
		}
		...
		/**
     * Render product accordion layout preview
     */
    public function shopglut_render_accordion_preview( $layout_id = 0 ) {
        ...
        if ( false === $layout_data ) {
            ...
            $layout_data = $wpdb->get_row(
                $wpdb->prepare( "SELECT * FROM `{$wpdb->prefix}shopglut_accordion_layouts` WHERE id = %d", $layout_id )
            );
        ...
        $markup_class = 'Shopglut\\showcases\\Accordion\\templates\\' . $template_name . '\\' . $template_name . 'Markup';
        if ( ! class_exists( $markup_class ) ) {
            return '<div class="shopglut-preview-error">Markup class not found: ' . esc_html( $markup_class ) . '</div>';
        }

        // Get the style class
        $style_class = 'Shopglut\\showcases\\Accordion\\templates\\' . $template_name . '\\' . $template_name . 'Style';
        ...
        try {
            ...
            echo '<div class="shopglut-accordion-preview-wrapper">';
            echo '<style>.shopglut-product-accordion.template1 .accordion-modal { opacity: 1; visibility: visible; position: relative; }</style>';
	...
	public function show_action_messages() {
		...
		if ( isset( $_GET['deleted'] ) && $_GET['deleted'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Accordion layout deleted successfully.', 'shopglut' ) .
				'</p></div>';
		}
		...
		if ( isset( $_GET['duplicated'] ) && $_GET['duplicated'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Accordion layout duplicated successfully.', 'shopglut' ) .
				'</p></div>';
		}
	}
