...
class AccordionSettingsPage {

	public $menu_slug = 'product_accordion';
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		$layout_data = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct query required for custom table operation, no caching needed
			$wpdb->prepare( "SELECT layout_name, layout_template FROM %s WHERE id = %d", $table_name, $layout_id ) // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnsupportedIdentifierPlaceholder -- Using %s instead of %i for compatibility, table name escaped with esc_sql()
		);
	    ...
	    <form id="shopglut_shop_layouts" method="post" action="">
				...
				<?php 
				$shopg_cpage_nonce = wp_create_nonce( 'shopg_Accordion_layouts' ); 
				?>
				<input type="hidden" name="shopg_Accordion_layouts_nonce" value="<?php echo esc_attr( $shopg_cpage_nonce ); ?>">
			   ...
			   <div id="post-body" class="metabox-holder columns-2">
						...
						<div id="shopg-Accordion-layout-settings" class="postbox-container shopg-layout-settings-wrapper">
						...
						<div class="submitbox" id="submitpost">
								<div id="AccordionLayout-publishing-action" class="shopg-publishing-button">
									<button type="button" id="Accordion-reset-settings-button" class="btn btn-fullwidth btn-secondary"
									style=" background: #dc3545; color: white; border: none;">
									<?php echo esc_attr__( 'Reset', 'shopglut' ); ?>
									</button>
									<button type="button" name="publish" id="Accordion-save-layout-button" class="btn btn-fullwidth">
						...
						<div id="shopg-Accordion-layout-container" class="shopg-admin-edit-panel shopg-layout-container">
		...
		<style>
			...
			/* When loader is visible, prevent interaction with accordion demo */
			.loader-overlay:not([style*="display: none"]) ~ * .shopglut-product-accordion {
