...
class TabchooseTemplates {

	public function __construct() {
		add_action('admin_post_create_tab_layout', array($this, 'handleCreateTabEnhancement'));
		add_action('wp_ajax_get_tab_demo_content', array($this, 'handleGetTabDemoContent'));
	}

	public function loadTabTemplates() {
		$tab_names = [
			'tab1' => [
						...
						<div class="shopg-template-preview" data-template="<?php echo esc_attr($layout_template); ?>">
							...
							<div class="template-content product-demo">
								...
								<div class="template-html-container">
									...
									<div class="html-preview-background">
										<?php $this->renderTabPreview($layout_template); ?>
									</div>

									<!-- Foreground: View Demo Button with Overlay -->
									<div class="html-preview-overlay">
										<button type="button" class="demo-view-btn" onclick="openHtmlDemoModal('<?php echo esc_attr($layout_template); ?>', 'product-tab')">
							...
							<div class="template-footer">
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									<input type="hidden" name="action" value="create_tab_layout">
									<input type="hidden" name="layout_template" value="<?php echo esc_attr($layout_template); ?>">
									<?php wp_nonce_field('create_tab_layout_nonce', 'create_tab_layout_nonce'); ?>
		...
		.shopg-template-gallery .html-preview-background .tab-modal-content,
		.shopg-template-gallery .html-preview-background .shopglut-product-tab .tab-modal-content {
		...
		/* Override tab modal styles in preview */
		.shopg-template-gallery .html-preview-background .tab-modal {
			position: static !important;
		...
		/* Ensure the tab preview container is centered and fits properly */
		.shopg-template-gallery .html-preview-background .shopglut-tab-preview-container {
			position: absolute !important;
			display: flex !important;
			align-items: center !important;
			justify-content: center !important;
		...
		.shopg-template-gallery .html-preview-background .tab-modal,
		.shopg-template-gallery .html-preview-background .tab-modal-content,
		.shopg-template-gallery .html-preview-background .tab-inner {
			overflow: hidden !important;
			max-height: none !important;
		...
		.shopg-template-gallery .html-preview-background .shopglut-product-tab {
			overflow: hidden !important;
		...
		.shopg-template-gallery .html-preview-background .tab-gallery,
		.shopg-template-gallery .html-preview-background .tab-info {
			overflow: hidden !important;
			max-height: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .tab-modal-overlay {
			display: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .tab-close {
			display: none !important;
		}

		/* Fix Live Demo modal visibility issue - override template1 style conflicts */
		#htmlDemoModal .shopglut-product-tab.template1 .tab-modal {
			opacity: 1 !important;
			visibility: visible !important;
			position: fixed !important;
			display: flex !important;
				...
				if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
					mutation.addedNodes.forEach(function(node) {
						if (node.nodeType === 1) { // Element node
							...
							if (node.classList && node.classList.contains('shopglut-product-tab') ||
								(node.id && node.id === 'htmlDemoModal') ||
								(node.querySelector && node.querySelector('.shopglut-product-tab'))) {
		...
		// Keep image modal functionality for tab images
		function openImageModal(imageName) {
			const modal = document.getElementById('imageModal');
			const mainImage = document.getElementById('modalMainImage');
			const assetsUrl = '<?php echo esc_js(SHOPGLUT_URL . "global-assets/images/tab-templates/"); ?>';
	 ...
	 */
	private function renderTabPreview($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Tabs/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Tabs/templates/template1/template1Style.php';

		$markup = new \Shopglut\showcases\Tabs\templates\template1\template1Markup();
		$style = new \Shopglut\showcases\Tabs\templates\template1\template1Style();

		// Render styles inline for preview
		$style->dynamicCss(0);
		?>
		<div class="shopglut-tab-preview-container">
			<?php
			// Render the demo content directly from template1Markup
			// Get default settings for demo
			$settings = $this->getDefaultTabSettings();
			$markup->render_tab_demo($settings, 0);
			?>
		</div>
		<?php
	}
	...
	/**
	 * Get default Tab settings for demo
	 */
	private function getDefaultTabSettings() {
		return array(
			'autoplay' => false,  // Disabled for modal
			'autoplay_speed' => 3000,
			'show_dots' => true,
			'show_arrows' => true,
	 ...
	 */
	private function renderTabFullDemo($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Tabs/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Tabs/templates/template1/template1Style.php';
		...
		// Render the full demo tab
		$markup->render_tab_demo($settings, 0);
	}

	/**
	 * Handle AJAX request for tab demo content
	 */
	public function handleGetTabDemoContent() {
		// Verify nonce
		...
		$this->renderTabFullDemo($template_id);
		wp_die();
	}

	public function handleCreateTabEnhancement() {
		if (
			!isset($_POST['create_tab_layout_nonce']) ||
			!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['create_tab_layout_nonce'])), 'create_tab_layout_nonce') ||
	...
	// Use $wpdb->insert() with proper error handling and caching
			...
			$cache_key = 'shopglut_tab_layouts';
			wp_cache_delete($cache_key, 'shopglut');

			// Update the layout_name with the actual ID using $wpdb->prepare()
			$update_result = $wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Required for custom table operations, cache cleared above
				sprintf("UPDATE `%s` SET layout_name = %s WHERE id = %d", esc_sql($table_name), sanitize_text_field('Layout(#' . $layout_id . ')'), $layout_id) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Using sprintf with escaped table name, no prepare needed
			);
