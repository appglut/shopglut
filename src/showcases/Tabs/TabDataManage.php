...
class TabDataManage {
	...
	public function __construct() {
		// AJAX handlers for product tab save and reset
		add_action('wp_ajax_save_shopg_Tab_layoutdata', [$this, 'save_Tab_layout_data']);
		add_action('wp_ajax_reset_shopg_Tab_layout_settings', [$this, 'reset_Tab_layout_settings']);

		// Handle individual actions (delete, duplicate)
		add_action('admin_init', [TabListTable::class, 'handle_individual_actions']);

		// Handle success messages
		add_action('admin_notices', [$this, 'show_action_messages']);

		// Initialize tab button display
	}


	/**
	 * Save product tab layout data from AJAX
	 */
	public function save_Tab_layout_data() {
		...
		if (!isset($_POST['Tab_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['Tab_nonce'])), 'shopg_Tab_layouts')) {
		...
		$layout_id = isset($_POST['shopg_Tab_layoutid']) ? intval(sanitize_text_field(wp_unslash($_POST['shopg_Tab_layoutid']))) : 0;
		...
		$preview_html = $this->shopglut_render_tab_demo_preview($layout_id);

		wp_send_json_success(array(
			'message' => 'Product tab layout saved successfully',
			'layout_id' => $layout_id,
			'html' => $preview_html
		));
	}
	...
	/**
	 * Reset product tab layout settings to default
	 */
	public function reset_Tab_layout_settings() {
		// Check nonce
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'shopg_Tab_layouts')) {
			wp_send_json_error('Invalid nonce');
			return;
		}
		...
		$cache_key = "shopglut_tab_layout_{$layout_id}";
		$layout_data = wp_cache_get( $cache_key, 'shopglut_tab' );

		if ( false === $layout_data ) {
			$layout_data = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct query required for custom table operation, no caching needed
				sprintf("SELECT layout_template FROM `%s` WHERE id = %d", esc_sql($table_name), $layout_id) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Using sprintf with escaped table name and sanitized layout_id
			);

			// Cache the result for 30 minutes
			wp_cache_set( $cache_key, $layout_data, 'shopglut_tab', 30 * MINUTE_IN_SECONDS );
		}
		...
		/**
     * Render product tab layout preview
     */
    public function shopglut_render_tab_demo_preview( $layout_id = 0 ) {
        ...
        if ( false === $layout_data ) {
            ...
            $layout_data = $wpdb->get_row(
                $wpdb->prepare( "SELECT * FROM `{$wpdb->prefix}shopglut_tab_layouts` WHERE id = %d", $layout_id )
            );
        ...
        $markup_class = 'Shopglut\\showcases\\Tab\\templates\\' . $template_name . '\\' . $template_name . 'Markup';
        if ( ! class_exists( $markup_class ) ) {
            return '<div class="shopglut-preview-error">Markup class not found: ' . esc_html( $markup_class ) . '</div>';
        }

        // Get the style class
        $style_class = 'Shopglut\\showcases\\Tab\\templates\\' . $template_name . '\\' . $template_name . 'Style';
        ...
        try {
            ...
            echo '<div class="shopglut-tab-preview-wrapper">';
            echo '<style>.shopglut-product-tab.template1 .tab-modal { opacity: 1; visibility: visible; position: relative; }</style>';
	...
	public function show_action_messages() {
		...
		if ( isset( $_GET['deleted'] ) && $_GET['deleted'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Tab layout deleted successfully.', 'shopglut' ) .
				'</p></div>';
		}
		...
		if ( isset( $_GET['duplicated'] ) && $_GET['duplicated'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Tab layout duplicated successfully.', 'shopglut' ) .
				'</p></div>';
		}
	}
