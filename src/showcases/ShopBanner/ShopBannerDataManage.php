...
class ShopBannerDataManage {

	public function __construct() {
		// AJAX handlers for product shopbanner save and reset
		add_action('wp_ajax_save_shopg_ShopBanner_layoutdata', [$this, 'save_ShopBanner_layout_data']);
		add_action('wp_ajax_reset_shopg_ShopBanner_layout_settings', [$this, 'reset_ShopBanner_layout_settings']);

		// Handle individual actions (delete, duplicate)
		add_action('admin_init', [ShopBannerListTable::class, 'handle_individual_actions']);

		// Handle success messages
		add_action('admin_notices', [$this, 'show_action_messages']);

		// Initialize shopbanner button display
	}


	/**
	 * Save product shopbanner layout data from AJAX
	 */
	public function save_ShopBanner_layout_data() {
		...
		if (!isset($_POST['ShopBanner_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['ShopBanner_nonce'])), 'shopg_ShopBanner_layouts')) {
		...
		$layout_id = isset($_POST['shopg_ShopBanner_layoutid']) ? intval(sanitize_text_field(wp_unslash($_POST['shopg_ShopBanner_layoutid']))) : 0;
		...
		$preview_html = $this->shopglut_render_shopbanner_preview($layout_id);

		wp_send_json_success(array(
			'message' => 'Product shopbanner layout saved successfully',
			'layout_id' => $layout_id,
			'html' => $preview_html
		));
	}
	...
	/**
	 * Reset product shopbanner layout settings to default
	 */
	public function reset_ShopBanner_layout_settings() {
		// Check nonce
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'shopg_ShopBanner_layouts')) {
			wp_send_json_error('Invalid nonce');
			return;
		}
		...
		$cache_key = "shopglut_shopbanner_layout_{$layout_id}";
		$layout_data = wp_cache_get( $cache_key, 'shopglut_shopbanner' );
		...
		if ( false === $layout_data ) {
			...
			$layout_data = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Direct query required for custom table operation
				$wpdb->prepare(
					sprintf("SELECT layout_template FROM `%s` WHERE id = %d", esc_sql($table_name)), // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- Using sprintf for table name, expected 0 but proper placeholders are used
					$layout_id
				)
			);

			// Cache the result for 30 minutes
			wp_cache_set( $cache_key, $layout_data, 'shopglut_shopbanner', 30 * MINUTE_IN_SECONDS );
		}
		...
		/**
     * Render product shopbanner layout preview
     */
    public function shopglut_render_shopbanner_preview( $layout_id = 0 ) {
        ...
        if ( false === $layout_data ) {
            ...
            $layout_data = $wpdb->get_row(
                $wpdb->prepare( "SELECT * FROM `{$wpdb->prefix}shopglut_shopbanner_layouts` WHERE id = %d", $layout_id )
            );
        ...
        $markup_class = 'Shopglut\\showcases\\ShopBanner\\templates\\' . $template_name . '\\' . $template_name . 'Markup';
        if ( ! class_exists( $markup_class ) ) {
            return '<div class="shopglut-preview-error">Markup class not found: ' . esc_html( $markup_class ) . '</div>';
        }

        // Get the style class
        $style_class = 'Shopglut\\showcases\\ShopBanner\\templates\\' . $template_name . '\\' . $template_name . 'Style';
        ...
        try {
            ...
            echo '<div class="shopglut-shopbanner-preview-wrapper">';
            echo '<style>.shopglut-product-shopbanner.template1 .shopbanner-modal { opacity: 1; visibility: visible; position: relative; }</style>';
	...
	public function show_action_messages() {
		...
		if ( isset( $_GET['deleted'] ) && $_GET['deleted'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Banner layout deleted successfully.', 'shopglut' ) .
				'</p></div>';
		}
		...
		if ( isset( $_GET['duplicated'] ) && $_GET['duplicated'] === '1' ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
				esc_html__( 'Shop Banner layout duplicated successfully.', 'shopglut' ) .
				'</p></div>';
		}
	}
