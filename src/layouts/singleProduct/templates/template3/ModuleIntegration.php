...
class ModuleIntegration {
	 ...
	 * Note: Swatches are handled automatically via WooCommerce filter hooks
	 * This method is primarily for enabling/disabling the swatches module
	 */
	public function render_swatches() {
		...
		// Swatches are automatically rendered by the ProductSwatches module
		// via the woocommerce_dropdown_variation_attribute_options_html filter
		// No manual rendering needed here - just trigger the action hook
		do_action('shopglut_swatches_single_product', $this->product_id, $this->settings);
	}
	...
	public function render_badges() {
		...
		// Get all badges from database using the same method as BadgeDataManage
		global $wpdb;
		...
		$badges = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct query required for custom table operation
			sprintf("SELECT * FROM `%s` ORDER BY id DESC LIMIT %d", esc_sql($table_name), 10) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Using sprintf with escaped table name and validated limit
		);
		...
		foreach ($badges as $badge) {
			// Get badge settings using same structure as BadgeDataManage
			...
			// Check if badge is enabled using nested value extraction
			$enable_badge = $this->get_nested_value($badge_settings, 'enable_badge');
			if (empty($enable_badge)) {
				continue;
			}

			// Check if badge should display for this product AND meets conditions
			if (!$this->should_display_badge_for_product($badge_settings, $current_product_id)) {
				continue;
			}

			// Check if badge meets its type-specific conditions (sale, new, etc.)
			if (!$this->should_display_badge_by_conditions($badge_settings, $current_product_id)) {
				continue;
			}
	...
	private function should_display_badge_by_conditions($settings, $product_id) {
		...
		switch ($badge_type) {
			...
			case 'new':
				...
				return $days_since_creation <= intval($new_product_days);
	...
	private function get_nested_value($array, $key, $default = null) {
		...
		// Check for specific badge structure patterns
		if (isset($array['shopg_product_badge_settings']['product_badge-settings'][$key])) {
			return $array['shopg_product_badge_settings']['product_badge-settings'][$key];
		}
	...
	public function render_comparison() {
		...
		$comparison_settings = isset($layout_settings['shopg_product_comparison_settings_template3'])
			? $layout_settings['shopg_product_comparison_settings_template3']
			: array();
		...
		if (!empty($button_styling)) {
			...
			if (isset($button_styling['border_width'])) {
				$border_width = is_array($button_styling['border_width']) ? ($button_styling['border_width']['width'] ?? $button_styling['border_width']) : $button_styling['border_width'];
				$style_parts[] = 'border-width: ' . esc_attr($border_width) . 'px';
			}
			if (isset($button_styling['border_radius'])) {
				$border_radius = is_array($button_styling['border_radius']) ? ($button_styling['border_radius']['width'] ?? $button_styling['border_radius']) : $button_styling['border_radius'];
				$style_parts[] = 'border-radius: ' . esc_attr($border_radius) . 'px';
			}
			if (isset($button_styling['padding'])) {
				if (is_array($button_styling['padding'])) {
					$padding_top = $button_styling['padding']['top'] ?? '0';
					$padding_right = $button_styling['padding']['right'] ?? '0';
					$padding_bottom = $button_styling['padding']['bottom'] ?? '0';
					$padding_left = $button_styling['padding']['left'] ?? '0';
					$padding_unit = $button_styling['padding']['unit'] ?? 'px';
					$padding_value = "{$padding_top}{$padding_unit} {$padding_right}{$padding_unit} {$padding_bottom}{$padding_unit} {$padding_left}{$padding_unit}";
				} else {
					$padding_value = $button_styling['padding'];
				}
				$style_parts[] = 'padding: ' . esc_attr($padding_value);
			}
	...
	public function render_at_position($position, $module_type = null) {
		...
		$position_map = array(
			...
			'after_description' => array(
				'custom_fields' => 'enable_custom_fields',
			),
		...
		foreach ($position_map[$position] as $module => $setting_key) {
			...
			// Special handling for custom fields - they should always render when enabled at after_description
			if ($module === 'custom_fields' && $position === 'after_description') {
	 ...
	 *
	 * @param string $module_type Module type (wishlist, badges, comparison, custom_fields, swatches)
	 * @return bool
	 */
	private function is_module_enabled($module_type) {
		...
		if (isset($this->settings[$enable_key]) && $this->settings[$enable_key]) {
			return true;
		}
		...
		if (isset($this->settings['single-product-settings'][$module_type . '_integration'][$enable_key])) {
			return $this->settings['single-product-settings'][$module_type . '_integration'][$enable_key] === '1';
		}
