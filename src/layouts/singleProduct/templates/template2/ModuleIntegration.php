...
class ModuleIntegration {
	...
	public function render_comparison() {
		...
		$comparison_settings = isset($layout_settings['shopg_product_comparison_settings_template2'])
			? $layout_settings['shopg_product_comparison_settings_template2']
			: array();
	...
	public function render_at_position($position, $module_type = null) {
		...
		// NOTE: Wishlist module is NOT included here because it's handled via WooCommerce hooks
		// in the wishlist module itself (WishlistHooks.php). Adding it here would cause duplicates.
		$position_map = array(
		...
		foreach ($position_map[$position] as $module => $setting_key) {
			...
			// Skip wishlist module entirely - it's handled by hooks
			if ($module === 'wishlist') {
				continue;
			}
