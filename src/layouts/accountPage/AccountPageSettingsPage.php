...
class AccountPageSettingsPage {

	public $menu_slug = 'accountpage';

	public function __construct() {

	}

	public function loadAccountPageEditor() {
...
$table_name = $wpdb->prefix . 'shopglut_accountpage_layouts';
		...
		$layout_data = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct query required for custom table operation
			sprintf("SELECT layout_name, layout_template FROM `%s` WHERE id = %d", $escaped_table, absint($layout_id)) // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Using sprintf with escaped table name and validated parameter
		);
	...
	<form id="shopglut_shop_layouts" method="post" action="">
				...
				<?php 
				$shopg_cpage_nonce = wp_create_nonce( 'shopg_accountpage_layouts' ); 
				?>
				<input type="hidden" name="shopg_accountpage_layouts_nonce" value="<?php echo esc_attr( $shopg_cpage_nonce ); ?>">
				...
				<div class="shopglut_layout_contents">
					...
					<div class="shopglut_editor_header">
						...
						<div class="back-to-menu">
							...
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=shopglut_layouts&view=accountpage' ) ); ?>"
			   ...
			   <div id="post-body" class="metabox-holder columns-2">
						...
						<div id="shopg-productaccountpage-layout-settings" class="postbox-container shopg-layout-settings-wrapper">
						...
						<div class="submitbox" id="submitpost">
								<div id="productaccountpageLayout-publishing-action" class="shopg-publishing-button">
									<button type="button" id="productaccountpage-reset-settings-button" class="btn btn-fullwidth btn-secondary"
									style=" background: #dc3545; color: white; border: none;">
									<?php echo esc_attr__( 'Reset', 'shopglut' ); ?>
									</button>
									<button type="button" name="publish" id="productaccountpage-save-layout-button" class="btn btn-fullwidth">
						...
						<div id="shopg-productaccountpage-layout-container" class="shopg-admin-edit-panel shopg-layout-container">
		...
		<style>
			...
			/* When loader is visible, prevent interaction with accountpage demo */
			.loader-overlay:not([style*="display: none"]) ~ * .shopglut-product-accountpage {
